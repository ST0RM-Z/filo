...
// ── Tier 2: Windows OS Metadata (Zone.Identifier / PowerShell) ─────────
function getWindowsOSMetadataCategory(filePath) {
  try {
    const powershellCmd = `Get-Content -Path "${filePath}:Zone.Identifier" -ErrorAction SilentlyContinue`;
    const stdout = execSync(`powershell -Command "${powershellCmd}"`, { encoding: 'utf8' });
...
function getWindowsDestDir(category, filePath, homeDir) {
  const metaCategory = getWindowsOSMetadataCategory(filePath);
  if (metaCategory) {
...
function processDirectory(dirPath, homeDir, options, stats, logs) {
  ...
  items.forEach(item => {
    ...
    if (stat.isDirectory()) {
      // Tier 1: Protect existing subfolders in ~/Documents
      return;
    } else if (stat.isFile()) {
  ...
  timeClusters.forEach(cluster => {
    ...
    cluster.forEach(({ item, fullPath }) => {
      ...
      const finalDestDir = clusterDir || getWindowsDestDir(category, fullPath, homeDir);
...
function run(args, homeDir) {
  ...
  sourceDirs.forEach(target => processDirectory(target, homeDir, options, stats, sessionLogs));
